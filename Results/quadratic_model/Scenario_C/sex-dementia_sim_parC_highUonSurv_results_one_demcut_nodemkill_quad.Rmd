---
title: "Simulation Scenario C:  Collider bias with interaction (large effect of U on survival and cognitive intercept)"
subtitle: "One dementia cut point; quadratic time to dementia"
output: 
...
#Life table data
US_MF <- read_csv(here("Data", "US_cohort_table_MF_calcs.csv"), 
...
#Data Analyses
sim_results <-
  read_csv(here("Results", "quadratic_model", "Scenario_C", 
...
knitr::include_graphics(here("DAGs", "sex-dem_sim_parBandC_DAG_UonInt_only.png"))
```
<br>

In this scenario, sex/gender and U influence survival.  The effect of U on survival in this scenario induces collider bias. U also influences level of cognitive function at age 50 (the intercept in our case) and dementia incidence.  
...
Sex-specific survival distributions from ages 50-95 years were generated to match survival distributions from the US cohort life table (1919-1921 birth cohort.)  For individual $i$ in time interval $j$, time to death was generated as a random variable drawn from an exponential survival distribution based on the hazard function in Equation \@ref(eq:mort-hazard). 

\begin{equation}
h_{\text{death}_{ij}}(t) =  \lambda_{j}\exp\{\gamma_{1j}\text{female}_i + \gamma_2U_i + \gamma_3\text{U}_{j}\times\text{male}_{i}\},
(\#eq:mort-hazard)
\end{equation}

where $U_i \sim N(0, 1)$.  The values for each of the coefficients in the hazard function are displayed in Table \@ref(tab:mort-haz-coeff).  In the table, $\gamma_{10}-\gamma_{19}$ represent the age-specific effect of being female on mortality.  
...
kable(lambda_values, 
      caption = "Baseline hazard for males in each 5-year age band") %>% kable_styling()
```
...
all_cp_survival <- 
  mean_sim_results[variable_names$cp_alive_varnames[1:num_tests]]
female_cp_survival <- 
  mean_sim_results[variable_names$cp_alive_females_varnames[1:num_tests]]
male_cp_survival <- 
  mean_sim_results[variable_names$cp_alive_males_varnames[1:num_tests]]

#Netherlands data that we need
US_MF_survival <- US_MF %>% filter(Age >= 50)
US_M_survival <- US_M %>% filter(Age >= 50)
US_F_survival <- US_F %>% filter(Age >= 50)
...
plot_data <- tibble("age" = seq(50, 95, by = 5), 
                    ...
                    "cohort_all_survival" = all_cp50_survival, 
                    "cohort_female_survival" = female_cp50_survival, 
                    "cohort_male_survival" = male_cp50_survival) 

cp_plot_data <- tibble("age" = seq(55, 95, by = 5), 
                       "pub_all_cp_survival" = all_life_US$CP[-1], 
                       "pub_female_cp_survival" = female_life_US$CP[-1], 
                       "pub_male_cp_survival" = male_life_US$CP[-1],
                       "cohort_all_cp_survival" = all_cp_survival, 
                       "cohort_female_cp_survival" = female_cp_survival, 
                       "cohort_male_cp_survival" = male_cp_survival) 
```
...
```{r cp-survival-plots, fig.show="hold", out.width="50%", fig.cap=paste("Average simulated conditional survival probabilites from age 50 years across", runs, "iterations of sample generation")}
#Female survival plot
...
female_surv_check <- cbind(plot_data$pub_female_survival, 
  ...
  mutate("Difference (Sim - Pub)" = Simulated - Published) %>% t() %>% as.data.frame() %>%
...
```
```{r Numerical-survival-checks2}
female_cp_surv_check <- cbind(cp_plot_data$pub_female_cp_survival, 
                              cp_plot_data$cohort_female_cp_survival) %>% 
  as.data.frame() %>% set_colnames(c("Published", "Simulated")) %>% 
  mutate("Difference (Sim - Pub)" = Simulated - Published) %>% t() %>% as.data.frame() %>%
  set_colnames(c(age_labels$formatted_age_intervals))

kable(female_cp_surv_check, 
      caption = 
        "Cumulative survival for women conditional on survival to age 50") %>% kable_styling()

male_cp_surv_check <- cbind(cp_plot_data$pub_male_cp_survival, 
                         cp_plot_data$cohort_male_cp_survival) %>% 
  as.data.frame() %>% set_colnames(c("Published", "Simulated")) %>% 
  mutate("Difference (Sim - Pub)" = Simulated - Published) %>% t() %>% as.data.frame() %>%
  set_colnames(age_labels$formatted_age_intervals)

kable(male_cp_surv_check, 
      caption = 
        "Cumulative survival for men conditional on survival to age 50") %>% kable_styling()
```
...
Average dementia incidence rates per 1000 person years across `r runs` simulated samples are displayed in Table \@ref(tab:dem-inc-rates). The dementia incidence rates from the ACT study, used as reference, are displayed in the first line of the table. The average dementia hazard ratios across `r runs` simulated scenarios are displayed in Table \@ref(tab:hr-table), and the average incidence rate ratios are displayed in Table \@ref(tab:IRR-table).  In both cases, ratios are presented as female:male.
...
dem_table <- 
  tibble("ACT" = c(" ", " ", " ", 
                       round(ACT_inc_rates$Male_AD_1000PY, 3)), 
...
ACT_IRR_table <- 
  tibble("ACT" = " ", 
         "IRR (Women:Men)" = ACT_inc_rates$`AD_IRR_F:M`) %>% 
  t() %>% set_colnames(age_labels$formatted_age_intervals[4:nrow(age_labels)])

kable(ACT_IRR_table, caption = paste("Incidence rate ratios for dementia from the ACT study")) %>% kable_styling()
```
