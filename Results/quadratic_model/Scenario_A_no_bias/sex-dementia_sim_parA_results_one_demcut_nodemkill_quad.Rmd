...
demented <- dataset %>% filter(dem == 1) %>% 
  dplyr::select("dem", "dem_Cij", "dem_random", variable_names$dem_varnames)

#All-- across age-bands
num_dem <- nrow(demented)
prop_dem <- mean(dataset$dem)

num_dem_random <- sum(demented$dem_random)
prop_dem_random <- mean(demented$dem_random)

num_dem_Cij <- sum(demented$dem_Cij)
prop_dem_Cij <- mean(demented$dem_Cij)

both_types <- demented %>% filter(dem_Cij == 1 & dem_random == 1)
num_both <- nrow(both_types)
prop_both <- nrow(both_types)/nrow(demented)

#All-- age-band specific

```

```{r dem-type-table}
dem_type_colnames <- c("All ages", age_labels$formatted_age_intervals)
