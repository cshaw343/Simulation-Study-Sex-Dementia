---
title: "Simulation Scenario B2:  Large effect of U on survival and on cognitive intercept"
...
#One simulation run
dataset <- readRDS(here("Data", "dataset_B2_500000_20200223")) %>% 
  as.data.frame()

#Data Analyses
sim_results <- read_csv(here("Results", "Scenario_B", 
                             "sim_B2_1000_20191126.csv")) 
...
dem_table <- 
  ...
  set_rownames(c("ACT", "Simulation Scenario A", "W", "M", "Combined", 
                 "Simulation Scenario B2", "W", "M", "Combined"))
...
IRR_table <- 
  ...
  t() %>% 
  set_colnames(age_labels$formatted_age_intervals) %>% 
  set_rownames(c("ACT", "IRR (Women:Men)", "Simulation Scenario B2", 
                 "IRR (Women:Men)"))
