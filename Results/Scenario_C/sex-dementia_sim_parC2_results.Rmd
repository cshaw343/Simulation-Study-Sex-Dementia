---
title: "Simulation Scenario C:  High effect of U on survival (for men only) and on cognitive intercept"
...
knitr::include_graphics(here("DAGs", "sex-dem_sim_parBandC_DAG.png"))
```
<br>

In this scenario, sex influences survival while U influences survival for men, level of cognitive function at age 50 (the intercept for cognitive trajectories in our case), and dementia incidence.  Because U influences survival for men in this scenario, we expect differential dementia incidence rates in women compared to men (higher in women) which would be consistent with collider bias due to selective survival.
...
Table \@ref(tab:Lambda-values) shows the computed values of $\lambda_j$ for our hazard function.  The values for each of the coefficients in the hazard function are displayed in Table \@ref(tab:mort-haz-coeff).  In Table \@ref(tab:mort-haz-coeff), $\gamma_{10}-\gamma_{19}$ represent the age-specific effect of being a woman on mortality.  Note $\gamma_2$ is set to 0 in Simulation Scenario C signifying that in this scenario, U affects mortality for men only. 
...
```{r cp-survival-plots, fig.show="hold", out.width="50%", fig.cap=paste("Average simulated cumulative survival probabilites from age 50 years across", runs, "iterations of sample generation")}
...
```{r Numerical-survival-checks, eval = FALSE}
...
We generated two types of dementia: dementia based on cognitive decline, modeled using quadratic cognitive function trajectories (see Equation \@ref(eq:Cij)) and "random shock" dementia generated using a hazard function with baseline hazard of 0.007 only.  This hazard value was determined based on the dementia incidence rate presented in Table 2 of Tom, et al. [@Tom2015] for ages 65-70.  We felt that data were thinner in younger ages, making measurements less precise and making it more likely that the cases observed in younger age bands were due to other health events that resulted in dementia.  It is unlikely that the risk for these adverse events decreases with age, so we allowed individuals to get "random shock" dementia at this same rate in all age bands.  In each interval, time to random shock dementia was calculated for each individual.  If time to "random shock" dementia was less than the individual's survival time for that age band, they were diagnosed with "random shock" dementia.  

To generate dementia due to cognitive decline, we used an age-constant cut point for "dementia". The model for generating $C_{ij}$, the cognitive function for person $i$ at time $j$, is displayed in Equation \@ref(eq:Cij)

\begin{equation}
C_{ij}(t) = \beta_{00} + \zeta_{0i} + \beta_{01}\text{female}_i + \beta_{02}U_i + \varepsilon_{i} + (\beta_{10} + \zeta_{1i} + \beta_{11}\text{female}_i + \beta_{12}U_i)t + (\beta_{20} + \zeta_{2i} + \beta_{21}\text{female}_i + \beta_{22}U_i)t^2,
(\#eq:Cij)
\end{equation}
...
dem_table <- 
  ...
  set_rownames(c("ACT", "Simulation Scenario A", "W", "M", "Combined", 
                 "Simulation Scenario C", "W", "M", "Combined"))
...
IRR_table <- 
  ...
  t() %>% 
  set_colnames(age_labels$formatted_age_intervals) %>% 
  set_rownames(c("ACT", "IRR (Women:Men)", "Simulation Scenario C", 
                 "IRR (Women:Men)"))
...
kable(random_table, 
      caption = paste0("Proportion of incident dementia cases in each age band attributable to \"random shock\" overall and by sex/gender averaged across ", runs, " simulation runs.")) %>% kable_styling()
```

```{r dem-type-table-Cij}
#Temp code for variable name mix up
Cij_table <- 
  rbind(c(mean_sim_results["prop_dem_Cij"], 
          mean_sim_results[
            variable_names$prop_dem_Cij_by_age[-c(1:3, num_tests + 1)]]), 
          c(mean_sim_results["prop_dem_Cij_W"], 
            mean_sim_results[paste0(variable_names$prop_dem_random_W_by_age,
                                    "_1")[-c(1:3, num_tests + 1)]]), 
          c(mean_sim_results["prop_dem_Cij_M"], 
            mean_sim_results[paste0(variable_names$prop_dem_random_M_by_age,
                                    "_1")[-c(1:3, num_tests + 1)]])) %>% 
          set_colnames(dem_type_colnames) %>% 
          set_rownames(dem_type_rownames)

kable(Cij_table, 
      caption = paste0("Proportion of incident dementia cases in each age band attributable to cognitive decline overall and by sex/gender averaged across ", runs, " simulation runs.")) %>% kable_styling()
...
#           c(mean_sim_results["prop_dem_Cij_W"], 
#             mean_sim_results[
#               variable_names$prop_dem_Cij_W_by_age[-c(1:3, num_tests + 1)]]), 
#           c(mean_sim_results["prop_dem_Cij_M"], 
#             mean_sim_results[
#               variable_names$prop_dem_Cij_M_by_age[-c(1:3, 
#                                                          num_tests + 1)]])) %>% 
#           set_colnames(dem_type_colnames) %>% 
#           set_rownames(dem_type_rownames)
# 
...
kable(both_table, 
      caption = paste0("Proportion of incident dementia cases in each age band attributable to both \"random shock\" and cognitive decline overall and by sex/gender averaged across ", runs, " simulation runs.")) %>% kable_styling()
```
...
dem_inc_by_sex <- dataset %>%
  ...
  set_colnames(age_labels$formatted_age_intervals[-c(1:2)])
...
ggplot(aes(Age, IRR), data = IRRs_by_year) + 
  ...
  ylim(0.990, 1.25) + 
  #scale_y_continuous(breaks = seq(0.990, 1.25, by = 0.005)) + 
