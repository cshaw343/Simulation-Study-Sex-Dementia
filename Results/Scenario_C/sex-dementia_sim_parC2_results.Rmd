---
title: "Simulation Scenario C2:  High effect of U on survival (for men only) and on cognitive intercept"
...
#One simulation run
dataset <- readRDS(here("Data", "dataset_C2_500000_20200223")) %>% 
  as.data.frame()

#Data Analyses
sim_results <- read_csv(here("Results", "Scenario_C", 
                             "sim_C2_1000_20191126.csv")) 
...
dem_table <- 
  ...
  set_rownames(c("ACT", "Simulation Scenario A", "W", "M", "Combined", 
                 "Simulation Scenario C2", "W", "M", "Combined"))
...
IRR_table <- 
  ...
  t() %>% 
  set_colnames(age_labels$formatted_age_intervals) %>% 
  set_rownames(c("ACT", "IRR (Women:Men)", "Simulation Scenario C2", 
                 "IRR (Women:Men)"))
