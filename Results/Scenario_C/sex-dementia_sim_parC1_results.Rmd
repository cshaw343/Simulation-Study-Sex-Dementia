---
title: "Simulation Scenario C1:  Medium effect of U on survival (for men only) and on cognitive intercept"
...
#One simulation run
dataset <- readRDS(here("Data", "dataset_C1_500000_20200223")) %>% 
  as.data.frame()

#Data Analyses
sim_results <- read_csv(here("Results", "Scenario_C", 
                             "sim_C1_1000_20191129.csv")) 
...
To generate dementia due to cognitive decline, we used an age-constant cut point for "dementia". The model for generating $C_{i}$, the cognitive function for person $i$ at time $j$, is displayed in Equation \@ref(eq:Cij)
...
where $U_i \sim N(0,1)$. Here, $\varepsilon_{i} \sim N(0, \sigma^2_{\varepsilon})$ represents unexplained variation in $C_{i}$.
...
dem_table <- 
  ...
  set_rownames(c("ACT", "Simulation Scenario A", "W", "M", "Combined", 
                 "Simulation Scenario C1", "W", "M", "Combined"))
...
IRR_table <- 
  ...
  t() %>% 
  set_colnames(age_labels$formatted_age_intervals) %>% 
  set_rownames(c("ACT", "IRR (Women:Men)", "Simulation Scenario C1", 
                 "IRR (Women:Men)"))
