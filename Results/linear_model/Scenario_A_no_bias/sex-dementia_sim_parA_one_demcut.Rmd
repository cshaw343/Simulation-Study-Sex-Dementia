...
subtitle: "Using one dementia cutpoint"
output: 
...
#One simulation run
dataset <- readRDS(here("Data", "dataset_A_onedemcut_500000_20190702")) %>% 
  as.data.frame()
...
mean_sim_results <- colMeans(sim_results, na.rm = TRUE)
...
```{r HR-Plots, fig.align = "center", out.width = "75%", fig.cap = "Female:Male mortality hazard ratios calculated from life tables for Denmark, the Netherlands, and France (three of the primary countries included in the Eurodem study). Ages on the x-axis represent ages at the start of 5-year age intervals."}
...
ggplot(HR_plot_data, aes(Age, HR)) + 
  ...
  labs(y = "Mortality Hazard Ratio (Female:Male)", x = "Age", 
       color = "") + ylim(0, 0.8) + theme_minimal() + 
  scale_y_continuous(breaks = seq(0, 0.8, 0.2)) + 
  scale_x_continuous(breaks = seq(0, 95, 5)) + 
  #xlim(50, 95) + 
  coord_cartesian(xlim = c(50, 95), ylim = c(0.4, 0.8)) +
...
\begin{equation}
h_{\text{death}_{ij}}(t) =  \lambda_{j}\exp\{\gamma_{1j}\text{female}_i + \gamma_2U_i + \gamma_3\text{slope}_{ij} + \gamma_4\text{C}_{ij}\},
(\#eq:mort-hazard)
\end{equation}
...
```{r mort-haz-coeff}
coeff_table <- 
  tibble("Simulation Scenario" = rep("", 12), 
         "A" = c(g1, g2, g3, g4)) %>% 
  t() %>% 
...
Using R's optim function [@Roptim], we solved for the $\lambda_j$ values (baseline mortality hazard for men in each 5-year age band) in our hazard function so that cumalitive probabilities of survival conditional on age 50, i.e. survival to age X, X > 50 conditional on survival to age 50, in our simulated data closely matched those calculated from the Netherlands cohort life tables.  
...
\begin{equation}
C_{ij}(t) = \beta_{00} + \zeta_{0i} + \beta_{01}\text{female}_i + \beta_{02}\text{age0_c50}_{i} + \beta_{03}U_i + \varepsilon_{ij} + (\text{slope}_{ij} + \zeta_{1i} + \beta_{11}\text{female}_i + \beta_{12}\text{age0_c50} + \beta_{13}U_i)t,
(\#eq:Cij)
\end{equation}
...
If an individual's value of cognitive function fell below the age-constant dementia cut point, we solved Equation \@ref(eq:dem-onset) for $t$ to get their time to dementia.  The age at which an individual is diagnosed with dementia is calculated as their age at the start of the interval plus their time to dementia within that interval.
...
Cij_check <- dataset %>% dplyr::select(female, variable_names$Cij_varnames)
...
```{r mean-slopes}
slopes <- colMeans(rbind(female_slopes_check[-1], male_slopes_check[-1])) %>%
  as.data.frame() %>% t() %>%
  set_colnames(na.omit(variable_names$interval_ages)) %>% set_rownames("")

kable(slopes, caption = "Observed mean slopes at each interval.  
...
```{r slope-var}
slope_var <- round(cij_var1[-1], 5) %>% as.data.frame() %>% t() %>%
  set_colnames(na.omit(variable_names$interval_ages)) %>% set_rownames("")
