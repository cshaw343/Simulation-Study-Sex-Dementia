  ...
  for(j in 1:length(opt_lambdas)){
    if(j == 1){
      opt_lambdas[j:length(opt_lambdas)] = 
        optimise(survivors, interval = c(0.00996, 0.011), 
      ...
      opt_lambdas[j:length(opt_lambdas)] = 
        optimise(survivors, 
                 interval = c(opt_lambdas[j - 1], 1.55*opt_lambdas[j - 1]), 
      ...
      opt_lambdas[j:length(opt_lambdas)] = 
        optimise(survivors, 
                 interval = c(1.75*opt_lambdas[j - 1], 4*opt_lambdas[j - 1]), 
                 obs = sim_data_unexposed, pop_size = num_unexposed, 
                 cp = cp50_unexposed)$minimum
      Sij <- t(survival_temp(obs_matrix = t(sim_data_unexposed), 
                             lambda = opt_lambdas))
  ...
  for(j in 1:length(opt_log_g1s)){
    if(j <= 1){
      opt_log_g1s[j:length(opt_log_g1s)] = 
        optimise(survivors, interval = c(-0.05, -0.047), obs = sim_data_exposed, 
