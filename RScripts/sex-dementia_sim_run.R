...
source(
  here("RScripts", 
       "sex-dementia_sim_parB_onedemcut_uniform_timetodem_nodemkill_UonInt.R"))  #The parameter file
...
data_gen(500000) %>%
  saveRDS(here(
    "Data", 
    "dataset_B_onedemcut_uniform_timetodem_nodemkill_UonInt_500000_20190823"))
...
sim_results <- parSapply(cl, 1:runs, function(i) {sex_dem_sim(100000)}) %>% 
  t() %>% as.data.frame() %>% 
  write_csv(
    here("Results", "Scenario_B", 
         "one_demcut_uniform_timetodem_nodemkill_UonInt_1000_20190823.csv"))
